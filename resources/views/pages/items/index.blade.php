    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-1 pt-6 pb-0">
            ...
            <div class="card-toolbar">
                <a data-fancybox data-type="ajax" data-src="{{route('items.create')}}" class="btn btn-primary btn-sm"
                   href="javascript:;">
            ...
            <table class="table table-bordered table-hover table-checkable mt-10" id="kt_datatable">
                ...
                @foreach($data as $key=>$item)
                    <tr>
                        ...
                        <td>
                            @if($item->status_id == 1)
                                <span class="label label-lg font-weight-bold  label-inline">Active</span>
                            @else
                                <span class="label label-lg font-weight-bold label-inline">Inactive</span>
                            @endif
                        </td>
                        <td>
                            <a href="#" class="btn btn-icon btn-success btn-xs mr-2 edit" data-fancybox data-type="ajax"
                               data-src="{{route('items.edit',$item->id)}}" data-toggle="tooltip" title="Edit"><i
                                        class="fa fa-pen"></i></a>
                            <form action="{{ route('items.destroy', $item->id) }}" style="display: inline-block;"
                                  method="post">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
