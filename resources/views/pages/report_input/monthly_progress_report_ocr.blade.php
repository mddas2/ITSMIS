...
<div class="card card-custom gutter-b">
 	...
 	<div class="card-header flex-wrap border-0 pt-6 pb-0">
        <div class="card-title">
         ...
         <div class="card-toolbar">
			<a data-fancybox data-type="ajax" data-src="{{route('indicator_ocr.create')}}" class="btn btn-primary btn-sm" href="javascript:;"><i class="fa fa-plus icon-sm"></i>New Indicators</a>&nbsp;
	...
	<div class="card-body">
		...
		<form class="form" id="kt_form" action="{{route('monthly_progress_report_ocr',['fiscal_year_id'=> $fiscalYearId])}}" method="post">
	 		{{csrf_field()}}
	            ...
	            <tbody id="tb_id">
	                ...
	                <tr>
	                    ...
	                    <td>
	                    	...
	                    	<input type="hidden" name="data[{{$key}}][indicators_id]" value="{{$row['indicators_id']}}" class="form-control" >
	                    	<input type="text" name="data[{{$key}}][name]" value="{{$row['name']}}" class="form-control" autocomplete="off">
	                    </td>
	                    <td>
	                    	<input type="text" name="data[{{$key}}][number]" value="{{$row['number']}}" class="form-control" autocomplete="off">
						</td>
						<td>
	                    	<input type="text" name="data[{{$key}}][progress]" value="{{$row['progress']}}" class="form-control" autocomplete="off">
						</td>
	                    <td>
