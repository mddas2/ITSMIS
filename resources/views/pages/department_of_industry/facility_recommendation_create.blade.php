    ...
    <div class="card card-custom gutter-b">
        ...
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
                <h3 class="card-label">
                    Department Of Industry - Facility Recommendation
                </h3>
            </div>
            <div class="card-toolbar">
                <a class="btn btn-success btn-sm" href="javascript:;" data-fancybox data-type="ajax" data-src="{{route('facility_recommendation_excel')}}" ><i class="fa fa-plus icon-sm"></i>{{ __('Import Excel')}}</a>
            </div>
        </div>
        <div class="card-body">
            <form>
            ...
            <form class="form" id="kt_form" action="{{route('facility_recommendation')}}" method="post">
                {{csrf_field()}}
                <table class="table table-bordered table-hover table-checkable mt-10" id="kt_datatable">
                    ...
                    @foreach($data as $row)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="text" name="data[{{$key}}][date_of_recomendation]"   data-single="true" class="form-control nepdatepicker" autocomplete="off" id="nep{{$key}}" value="{{$row->date_of_recomendation}}" {{$disabled}}>
                            </td>
                            <td>
                                <input type="text" name="data[{{$key}}][name_of_industry]" class="form-control" autocomplete="off" value="{{$row->name_of_industry}}" {{$disabled}}>
                            </td>
                            <td>
                                <input type="text" name="data[{{$key}}][type_of_recomendation]" class="form-control" autocomplete="off" value="{{$row->type_of_recomendation}}" {{$disabled}}>
                            </td>
                    ...
                    <tr id="firstRow">
                        ...
                        <td>
                            ...
                            <input type="text" name="data[{{$key}}][date_of_recomendation]"   data-single="true" class="form-control nepdatepicker" autocomplete="off" >
                        </td>
                        <td>
                            <input type="text" name="data[{{$key}}][name_of_industry]" class="form-control" autocomplete="off" >
                        </td>
                        <td>
                            <input type="text" name="data[{{$key}}][type_of_recomendation]" class="form-control" autocomplete="off">
                        </td>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        $('.add').click(function(e){
            ...
            $("[name='data["+key+"][date_of_recomendation]']",rowClone).val("");
            $("[name='data["+key+"][date_of_recomendation]']",rowClone).attr('id',"nepstart"+tableCnt+1);
            $("[name='data["+key+"][name_of_industry]']",rowClone).val("");
            $("[name='data["+key+"][type_of_recomendation]']",rowClone).val("");
            $("[name='data["+key+"][locked]']",rowClone).val("");

            $("[name='data["+key+"][id]']",rowClone).attr('name','data['+tableCnt+'][id]');
            $("[name='data["+key+"][date_of_recomendation]']",rowClone).attr('name','data['+tableCnt+'][date_of_recomendation]');
            $("[name='data["+key+"][name_of_industry]']",rowClone).attr('name','data['+tableCnt+'][name_of_industry]');
            $("[name='data["+key+"][type_of_recomendation]']",rowClone).attr('name','data['+tableCnt+'][type_of_recomendation]');
