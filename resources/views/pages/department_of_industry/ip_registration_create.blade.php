    ...
    <div class="card card-custom gutter-b">
        ...
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            ...
            <div class="card-toolbar">
                <a class="btn btn-success btn-sm" href="javascript:;" data-fancybox data-type="ajax" data-src="{{route('ip_registration_excel')}}" ><i class="fa fa-plus icon-sm"></i>{{ __('Import Excel')}}</a>
            </div>
        </div>
        <div class="card-body">
            <form>
                ...
                <table class="table table-bordered table-hover table-checkable mt-10" id="kt_datatable">
                    ...
                    @foreach($data as $row)
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="data[{{$key}}][name_of_industry_person]" class="form-control" autocomplete="off" value="{{$row->name_of_industry_person}}" {{$disabled}}>
                            </td>
                            <td>
                                <input type="text" name="data[{{$key}}][type_of_ip]" class="form-control" autocomplete="off" value="{{$row->type_of_ip}}" {{$disabled}}>
                            </td>
                    ...
                    <tr id="firstRow">
                        ...
                        <td>
                            <input type="text" name="data[{{$key}}][name_of_industry_person]" class="form-control" autocomplete="off" >
                        </td>
                        <td>
                            <input type="text" name="data[{{$key}}][type_of_ip]" class="form-control" autocomplete="off">
                        </td>
...
@section('scripts')
    ...
    <script type="text/javascript">
        $('.ipRegistration').addClass("active");
        ...
        $('.add').click(function(e){
            ...
            $("[name='data["+key+"][name_of_industry_person]']",rowClone).val("");
            $("[name='data["+key+"][type_of_ip]']",rowClone).val("");
            ...
            $("[name='data["+key+"][name_of_industry_person]']",rowClone).attr('name','data['+tableCnt+'][name_of_industry_person]');
            $("[name='data["+key+"][type_of_ip]']",rowClone).attr('name','data['+tableCnt+'][type_of_ip]');
