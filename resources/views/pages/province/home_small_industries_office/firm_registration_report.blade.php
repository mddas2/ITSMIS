	...
	<div class="card-body">
		<ul class="dashboard-tabs nav nav-pills nav-primary row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
			<li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
				<a class="nav-link border py-2 d-flex flex-grow-1 rounded flex-column align-items-center marketMonitoring" href="{{route('dcsc-market-monitoring-report')}}">
			...
			<li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
				<a class="nav-link border py-2 d-flex flex-grow-1 rounded flex-column align-items-center firmRegistration" href="{{route('dcsc_firm_registration_report')}}">
					<span class="nav-icon py-3 w-auto">
						<span class="svg-icon svg-icon-3x">
			...
			<li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
				<a class="nav-link border py-2 d-flex flex-grow-1 rounded flex-column align-items-center importExportPage" href="{{route('permission_import_export')}}">
					<span class="nav-icon py-3 w-auto">
						<span class="svg-icon svg-icon-3x">
							<i class="fas fa-poll-h icon-2x"></i>
						</span>
 	...
 	<div class="card-header flex-wrap border-0 pt-6 pb-0">
        ...
        <div class="card-toolbar">
        	<a class="btn btn-info btn-sm" href="javascript:;" data-fancybox data-type="ajax" data-src="{{route('hasio_firm_registration_import_column')}}" ><i class="fa fa-plus icon-sm"></i>{{ __('Import New Column')}}</a>
		</div>
    </div>
	<div class="card-body">
		<form>
	            ...
	            <tbody id="tb_id">
	                ...
	                <tr>
	                    ...
	                    <td>
	                   	...
	                   	<td>{{$row->user->name}}</td>
	                    <td>
	                    	{{$row->no_of_registered_firm}}
						</td>
						<td>
	                    	{{$row->type_of_registered_firm}}
						</td>
						<td>
	                    	{{$row->no_of_firm_renewal}}
						</td>
						<td>
	                    	{{$row->type_of_firm}}
						</td>
						<td>
	                    	{{$row->firm_cancellation}}
						</td>
						<td>
	                    	{{$row->revenue_raised}}
						</td>
	                    ...
	                    <td>
	                    	<?php 
			                	if ($row->locked == 1) { ?>
			                		<a class="btn btn-danger btn-sm" href="{{route('dcsc_market_monitoring_lock',['lock' => 0,'id'=>$row->id])}}" data-toggle="tooltip" title="Unlock Data">
