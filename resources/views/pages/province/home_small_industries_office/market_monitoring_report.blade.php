	...
	<div class="card-body">
		<ul class="dashboard-tabs nav nav-pills nav-primary row row-paddingless m-0 p-0 flex-column flex-sm-row" role="tablist">
			<li class="nav-item d-flex col-sm flex-grow-1 flex-shrink-0 mr-3 mb-3 mb-lg-0">
				<a class="nav-link border py-2 d-flex flex-grow-1 rounded flex-column align-items-center marketMonitoring" href="{{route('hasio-market-monitoring-report')}}">
...
<div class="card card-custom">
 	<div class="card-header">
        <div class="card-title">
            <h3 class="card-label">
            	Market Monitoring - Home And Small Industries Office
        	</h3>
        </div>
        <div class="card-toolbar">
        	<a class="btn btn-info btn-sm" href="javascript:;" data-fancybox data-type="ajax" data-src="{{route('hasio_market_monitoring_import_column')}}" ><i class="fa fa-plus icon-sm"></i>{{ __('Import New Column')}}</a>
		</div>
    </div>
	<div class="card-body">
		<form>
	        ...
	        <tbody id="tb_id">
	            ...
	            <tr>
	                ...
	                <td>
	                	<?php 
		                	if ($row->locked == 1) { ?>
		                		<a class="btn btn-danger btn-sm" href="{{route('hasio_market_monitoring_lock',['lock' => 0,'id'=>$row->id])}}" data-toggle="tooltip" title="Unlock Data">
